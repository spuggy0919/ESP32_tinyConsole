...
#undef RP2040BOARD
#undef RP2040BOARD2
#define ESP32BOARD		/*spuggy0919*/						/** spuggy0919 **/
#undef MKR1010BOARD
...
#if defined(RP2040LITTLEFS) || defined(ESPSPIFFS) || defined(ARDUINOSD) || defined(ESPLITTLEFS)  /*spuggy0919*/
...
void consins(char *b, short nb) {
	...
	while(z.a < nb) {
  		c=inch(); 
		// Serial.printf("consins[%2x]%c\n",c,c); /*spuggy0919*/
  		if (id == ISERIAL || id == IKEYBOARD){
			// outch(c); /*spuggy0919*/ no need to echo command
			// Serial.printf("id==ISERIAL[%2x]%c\n",c,c); /*spuggy0919*/
		} 
		if (c == '\x03' || c == '\x1b') { /*spuggy0919 for exit interactive mode*/
			b[z.a++]=c; break;
		}
