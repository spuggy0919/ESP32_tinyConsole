 ...
 */
//#define ARDUINO /*spuggy0919*/
#ifdef ARDUINO
#include "hardware-arduino-HTTP.h" /*spuggy0919*/
#else 
...
#endif
				return;
#if !defined(ARDUINO) || defined(ARDUINORTC) || defined(ESP32RTC) /*spuggy0919*/
			case 'T':
				if (m == 'g') *v=rtcget(i); 
...
//#ifndef ARDUINO /*spuggy0919*/
    if ( c == '@' && d == 'A' ) {
        k=0;
        if (bargc > 1) while(k < SBUFSIZE && bargv[1][k] !=0) { sbuffer[k]=bargv[1][k]; k++; } 
        return sbuffer+b;					/* modified from bargv[2] to bargv[1]*/
    }
// #endif/*spuggy0919*/
...
#ifdef ARDUINOWIRE
		case IWIRE:
			Serial.printf("IWIRE"); /*spuggy0919*/
			ins(sbuffer, 1);
...
#ifdef ARDUINOMQTT
    case IMQTT:
			Serial.printf("IMQTT"); /*spuggy0919*/
    	return mqttinch();
#endif
#ifdef ARDUINOPRT
		case ISERIAL1:
...
char checkch(){
	switch (id) {
		case ISERIAL:
			// Serial.printf("checkchISERIAL"); /*spuggy0919*/
...
short availch(){
	switch (id) {
    case ISERIAL:
	//   Serial.printf("availchISERIAL"); /*spuggy0919*/
 ...
 */
void outch(char c) {
 	// Serial.printf("outch"); /*spuggy0919*/
	switch(od) {
		case OSERIAL:
 	// Serial.printf("outchOSERIAL[%2x]%c\n",c,c); /*spuggy0919*/
			serialwrite(c);
			break;
#ifdef FILESYSTEMDRIVER
		case OFILE:
...
void outs(char *ir, address_t l){
	...
	switch (od) {
		...
		default:
			for(i=0; i<l; i++){
				 outch(ir[i]);
				// Serial.printf("outsoutch(od=[%2x]%c\n",ir[i],ir[i]); /*spuggy0919*/
...
/* after each statement we check on a break character  
		...
		if (checkch() == BREAKCHAR || checkch() == EXITCHAR) {st=SINT; xc=inch(); return;};  /*spuggy0919*/
